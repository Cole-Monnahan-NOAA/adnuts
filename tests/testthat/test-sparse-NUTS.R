  ...
  for(m in c('auto', 'dense', 'sparse', 'stan', 'sparse-naive', 'diag', 'unit')){
    suppressWarnings(suppressMessages(fits[[m]] <-
                sample_snuts(obj, num_samples=850,
                             ...
                             Q=Q, Qinv=M,
                             refresh=0, adapt_stan_metric = TRUE,
...
test_that("Embedded Laplace approximation works", {
  skip_if(skip_TMB)
  obj <- get_simple_obj()
  fit1 <- sample_snuts(obj, num_samples=800, refresh=0,
                            num_warmup=200, cores=1, chains=1, seed=1,
                            metric='dense', print=FALSE)
   # has RE and uses Laplace
  fit2 <- sample_snuts(obj, num_samples=800, laplace=TRUE, refresh=0,
                            num_warmup=200, cores=1, chains=1, seed=1,
                            metric='dense', print=FALSE)
...
test_that("metrics are robust to model type",{
  ...
  obj <- TMB::MakeADFun(data=obj$env$data, parameters=obj$env$parList(),
                        ...
                        DLL=obj$env$DLL)
  expect_error(sample_snuts(obj, num_samples=800, laplace=TRUE,
                            num_warmup=200, cores=1, chains=1, seed=1,
                            metric='dense', print=FALSE),
...
test_that("auto metric selection is robust to model type", {
  ...
  obj2 <- TMB::MakeADFun(data=obj$env$data, parameters=par0,
                        ...
                        random=NULL, silent=TRUE,
                        DLL=obj$env$DLL)
  expect_error(sample_snuts(obj2, num_samples=800, laplace=TRUE,refresh=0,
                                 num_warmup=200, cores=1, chains=1, seed=1,
                                 metric='auto', print=FALSE),
               regexp = 'No random effects found')
  # this breaks if init='last.par.best' b/c the inits are so bad it can't recover
  expect_warning(fit3 <- sample_snuts(obj2, num_samples=800, laplace=FALSE, init='unif',
                            num_warmup=200, cores=1, chains=1, seed=1,
                            refresh=0, print=FALSE, adapt_stan_metric = TRUE,
  ...
  obj2 <- TMB::MakeADFun(data=obj$env$data, parameters=par0,
                         ...
                         random=NULL, silent=TRUE,
                         DLL=obj$env$DLL)
  expect_error(sample_snuts(obj2, num_samples=800, laplace=TRUE,
                                 num_warmup=200, cores=1, chains=1, seed=1,
                                 metric='auto', print=FALSE),
               regexp = 'No random effects found')
...
test_that("RTMB works", {
  ...
  #obj$fn()
  fit <- sample_snuts(obj, num_samples=750, num_warmup=250, chains=2,
...
test_that("small models work", {
  ...
  fit <- sample_snuts(obj, num_samples=100, num_warmup=200, chains=1,
 ...
 # repeat with 2 chains
  fit <- sample_snuts(obj, num_samples=100, num_warmup=200, refresh=0,
...
test_that("num_samples and num_warmup work", {
  ...
  f(params)
  obj <- RTMB::MakeADFun(f, params)
  expect_warning(fit1 <- sample_snuts(obj, iter=400, warmup=200,
                       chains=1,
                      refresh=0, cores=1, seed=1, print=FALSE),
                 regexp = 'iter and warmup are deprecated')
  expect_warning(fit1 <- sample_snuts(obj, iter=400, warmup=NULL,
                                      chains=1,
                                      refresh=0, cores=1, seed=1, print=FALSE),
                 regexp = 'iter and warmup are deprecated')
  fit1 <- sample_snuts(obj,
                       chains=1,
                       refresh=0, cores=1, seed=1, print=FALSE)
  expect_true(fit1$warmup==150)
  expect_true(fit1$iter==1000)
  #plot_sampler_params(fit1)
  fit1 <- sample_snuts(obj, metric='stan',
                       chains=1,
                       refresh=0, cores=1, seed=1, print=FALSE)
