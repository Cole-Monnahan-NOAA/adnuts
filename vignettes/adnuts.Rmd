...
- Run no-U-turn sampler (NUTS) or random walk Metropolis (RWM) MCMC chains
  from within R using the `sample_admb` functions.
...
model name and path. It works with two MCMC algorithms, NUTS and RWM, which
are built into the ADMB source code so this is just a wrapper
function. Also note that this function does not do optimization nor
...
guide](http://mc-stan.org/users/documentation/case-studies/rstan_workflow.html))
...
informative to run the RWM algorithm because it often provides fast
feedback from which the user can determine the cause of poor mixing (see
[@monnahan2019]). `sample_admb` can also run RWM chains via the argument
`algorithm="RWM"`. Consult the ADMB documentation for more information on a
workflow with these samplers. `adnuts` provides no new options compared to
the command line from previous ADMB versions (besides a better console
output), but the option for parallel execution and integration with MCMC
diagnostic tools should be sufficiently appealing to users. Once a model is
more appropriately parameterized, NUTS should be used.
...
values relative to the typical set of the posterior. `sample_admb` can
accept a list of lists (one for each chain), or function which returns a
list of parameters (e.g., `init <- function() list(a=rnorm(1),
...
Parallel sampling is done by default as of version 1.2.0. It is done by
...
session calls `sample_admb` and when done the results are merged together
...
`rstan::monitor`. This function is automatically run for completed
`sample_admb` runs and stored in the output. For very large models (either
many parameters or many iterations) this operation can be slow and thus a
user may disable it with the argument `skip_monitor`, however this
situation should be rare as these quantities should always be checked.
...
extracted as a data frame. These functions have optional arguments for
...
The object returned by `sample_admb` can also be plugged directly into the
...
adnuts may link these more directly. But for now it can be done manually
such as with the energy diagnostic:
...
Looking at the last row for parameter "sigmaphi" which is a hypervariance,
the last plot shows the traces of two chains (colors), where alternative
...
pairwise posterior samples (black points) for the remaining parameters. A
red point shows the posterior mode and an ellipse shows the 95% bivariate
confidence region, taken from the inverse Hessian calculated by
...
estimation during warmup (adaptation). The initial matrix can likewise
easily be intialized in two ways. First is a unit diagonal, and second is
the "MLE" option, which more accurately is the covariance matrix calculated
from inverting the Hessian at the maximum posterior density (the mode --
informally referred to as the MLE often). I refer to this as $\Sigma$. As
such there are 6 options for the mass matrix, summarized in the subsequent
table. Note that options 3 and 6 were not available before version 1.2.0
and are only available for ADMB >12.0.
...
| Unit  | Dense    | `adapt_mass_dense=TRUE`| `-mcdiag`<br>`-adapt_mass_dense`| Use when strong correlations but $\Sigma$ is unavailable and $d$<5000  |
| $\Sigma$  | None | `metric='mle'`<br>`adapt_mass=FALSE`| (default) | When $\Sigma$ well approximates the posterior | 
| $\Sigma$  | Diagonal | `metric='mle'`| `-mcdiag`<br>`-adapt_mass` | When $\Sigma$ is poor approximation but $d$>5000. 
| $\Sigma$  | Dense |  `metric='mle'`<br>`adapt_mass_dense=TRUE`| `-adapt_mass_dense`| When $\Sigma$ is poor approximation but $d$<5000 
...
Furthermore, it is often hard to estimate a good dense mass matrix,
especially in the first few phases of warmup with very few samples. In such
cases the Cholesky decomposition of the estimated matrix may fail. Instead
of crashing the run I coded ADMB to instead do a diagonal estimate in this
case, and try a dense update at the next phase, repeating until warmup is
over. Warnings are printed to the console when this happens.
...
## Sampling for TMB models Previous versions of `adnuts` included explicit
